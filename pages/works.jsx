...
const Works = () => {
	...
	return (
		<section className={`${styles.works} works__bg`}>
			<NextHead
				title="Works | Emil Murahas"
				previewText="Work from the simplest to complex full-stack development."
				content="website"
			/>
			<div className={styles.container}>
				...
				<div className={styles.works__content}>
					...
					<p className={styles.works__subtitle} data-aos="zoom-in">
						Here you can look at my pet projects, as well as commercial ones, unfortunately not all projects
						have been released or can be shown.
					</p>
					<div className={styles.list}>
						{projects.map((item, index) => (
							<div className={styles.list__item} key={index}>
								<div
									className={`${styles['list__item-info']} background-dark`}
									data-aos="zoom-in"
									data-aos-duration="1200"
								>
									<div className={styles['list__item-top']}>
										<h3 className={styles['list__item-title']}>{item.title}</h3>
										<div className={styles['list__item-links']}>
											<a
												href={item.link}
												target="_blank"
												rel="noreferre"
												aria-labelledby={`go-to-link-default-${index}`}
												className={[styles.list__link, styles['list__link-default']].join(' ')}
											>
												<svg
												...
												<i
													className={styles['list__link-text']}
											...
											<a
												...
												className={[styles.list__link, styles['list__link-youtube']].join(' ')}
											>
												<svg
													xmlns="http://www.w3.org/2000/svg"
													width="25"
											...
											<a
												...
												className={[styles.list__link, styles['list__link-github']].join(' ')}
											>
												<svg
													xmlns="http://www.w3.org/2000/svg"
													width="25"
									...
									<p className={styles['list__item-text']}>{item.text}</p>
									<ul className={styles.tags}>
										{item.tags.map((item, index) => (
											<li className={`${styles.tag} skills__item-dark`} key={index}>
												{item}
											</li>
										))}
									</ul>
								...
								<div
									className={styles['list__item-preview']}
									data-aos="zoom-in"
									data-aos-duration="1000"
								>
									<Image fill unoptimized={true} src={item.preview} alt="project preview" />
									<div className={`${styles['list__item-overlay']} overlay-dark`}>
